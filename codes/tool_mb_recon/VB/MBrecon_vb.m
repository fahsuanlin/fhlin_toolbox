function [ EPIsos,EPIrec ] = MBrecon_vb( filename1,filename2,lambda )
...
mrprot=ice_read_prot_mbsirepi_vb(sprintf('vb15_meas_04.asc'));
mrprot=ice_read_prot_mbsirepi_slices(sprintf('vb15_meas_04.asc'),mrprot);
mrprot=ice_read_prot_mbsirepi_FOV(sprintf('vb15_meas_01.asc'),mrprot);
fclose(file_in);
...
if mrprot.dInPlaneRot<pi/4
    ...
    esle
    EPIsos=temp(:,size(EPIsos,2)+1+PixelShift:size(EPIsos,2)+mrprot.lPhaseEncodingLines+PixelShift,:,:);
    EPIsos=permute(EPIsos,[2,1,3,4]);
    temp=repmat(EPIrec,[1,2,1,1]);
    EPIrec=temp(:,size(EPIsos,2)+1+PixelShift:size(EPIsos,2)+mrprot.lPhaseEncodingLines+PixelShift,:,:);
    EPIrec=permute(EPIrec,[2,1,3,4]);
    end
end
