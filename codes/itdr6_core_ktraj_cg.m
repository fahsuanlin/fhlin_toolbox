function [recon,b,delta,g2_gfactor,Local_kspace,l1_history,R_history]=itdr6_core_ktraj_cg(varargin);
%
%	itdr6_core_ktraj_cg		perform generalized PatLoc reconstruction using
%	time-domain signals and the conjugated gradient method in Cartesian
%	sampling trajectory
%
%
%	[recon,b,delta]=itdr6_core_ktraj_cg('Y',Y,'S',S,'G',G,'K',K,'flag_display',1);
%
%	INPUT:
...
%iteratively reweighted least squares for l-1 norm minimization
for l1_idx=1:n_l1_iteration    
    ...
    if(~flag_cg_gfactor)
        ...
        if(lambda>eps)
            switch(sparse_tx)
                case 'id' %identity
                    a=a+lambda.*lambda.*sqrt(R).*X0;
                case 'cs' %wavelet transform
                    [c,s] = wavedec2(X0,4,'db4');
                    %c=zeros(size(c));
                    a=a+lambda.*lambda.*sqrt(R).*waverec2(c,s,'db4');
                case 'del2' %discrete laplacian
                    a=a+lambda.*lambda.*sqrt(R).*del2(X0);
            end;
        end;
    ...
    %R_p=mean(abs(R(:)).^2);
    R=sqrt(eps+abs(sqrt(R).*b(:,:,end)).^2);
    %R=R./sqrt(R_p);
    R=R./(max(R(:)));
    
    lambda=0;
    R_history(:,:,l1_idx)=R;
    l1_history(:,:,l1_idx)=b(:,:,end).*sqrt(R);
