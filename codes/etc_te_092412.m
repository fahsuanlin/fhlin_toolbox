...
if(flag_self_information_latency)
    ...
    for i=1:min(100,floor(size(ROI,1)-1))    
        si_1(i)=etc_mi(ROI(i+1:end,1)',ROI(1:end-i,1)',n_bin,'edge_a',edge1,'edge_b',edge1);
        si_2(i)=etc_mi(ROI(i+1:end,2),ROI(1:end-i,2),n_bin,'edge_a',edge2,'edge_b',edge2);
    end;
...
if(flag_mutual_information_latency)
    if(flag_display)
        fprintf('estimating the latency for maximal mutual-information....\n');
    end;
    for i=1:min(100,floor(size(ROI,1)./2))    
        mi(i)=etc_mi(ROI(:,1)',ROI(:,2)',n_bin,'edge_a',edge1,'edge_b',edge2);
    end;
    [max_mi, max_mi_idx]=max(mi);
else
    max_mi_idx=0;
end;
...
if(flag_display)
    ...
    fprintf('max MI between time series 1 and 2 with lantecy [%d] samples...\n',max_mi_idx);
end;
...
if(isempty(edge1)|isempty(edge2))
    if(~isempty(timeVec))
        ...
        edge1(end)=max(ROI(base_idx,1));
        edge1(1)=min(ROI(base_idx,1));
        edge2(end)=max(ROI(base_idx,2));
        edge2(1)=min(ROI(base_idx,2));
