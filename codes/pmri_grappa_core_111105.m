...
for i=1:length(acc_token)
	...
	for j=1:length(acs_idx)
		...
		for k=1:size(freq_idx,2)
			...
			tmp=obs(acc_idx(idx),freq_idx(:,k),:);
    ...
    if(flag_reg)% regularized
        ...
        if(flag_common_coil)
            ...
            for coil_idx=1:size(obs,3)
                keyboard;
                pp=P(acs_idx,:,coil_idx);
                tmpp=v_e*Gamma*u_e'*G{i}.beta{coil_idx}+v_e*(Phi*v_e'*pp(:));
                %tmpp=v_e*(v_e'*pp(:));
                tmp=cat(3,tmp,reshape(tmpp,[size(freq_idx,2),length(acs_idx)]));
            end;
        ...
        if(flag_common_coil)
            ...
            tmp=[];
            for coil_idx=1:size(obs,3)
                tmp=cat(3,tmp,reshape(A*G{i}.beta{coil_idx},[size(freq_idx,2),length(acs_idx)]));
            end;
            recon=permute(tmp,[2,1,3]);    
        end;
    end;
