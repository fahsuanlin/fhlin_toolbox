...
for core=start_level:end_level
	...
	while(ch>threshold)
		...
		dwt_coef=cir_wavedec(n_recorded,core,h0.*sqrt(2), h1.*sqrt(2));
		...
		if(ndims(raw)==3)		
			appr_coef(1:size(raw,1)./2^(core),1:size(raw,1)./2^(core),1:size(raw,1)./2^(core))=dwt_coef(1:size(raw,1)./2^(core),1:size(raw,1)./2^(core),1:size(raw,1)./2^(core));
		end;
