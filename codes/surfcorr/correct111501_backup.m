...
if(~isempty(mask))
    mask_idx=find(mask);
    for j=1:size(recon_all,ndims(recon_all))
        if(ndims(raw)==3)
            recon_all(:,:,:,j)=recon_all(:,:,:,j).*mask;
        end;
        if(ndims(raw)==2)
            recon_all(:,:,j)=recon_all(:,:,j).*mask;
        end;
...
if(isempty(packet_fine_tune_level)|packet_fine_tune_level<1)
    ...
    if(~isempty(mask))
        for j=1:size(recon_all_packet,ndims(recon_all_packet))
            if(ndims(raw)==3)
                recon_all_packet(:,:,:,j)=recon_all_packet(:,:,:,j).*mask;
            end;
            if(ndims(raw)==2)
                recon_all_packet(:,:,j)=recon_all_packet(:,:,j).*mask;
            end;
    ...
    for i=1:size(recon_all_packet,ndims(recon_all_packet))
        ...
        [inhomo_packet(i),cluster_mean_packet(i,:),cluster_var_packet(i,:),cluster_population_packet(i,:),im_cluster_packet(i,:,:,:)]=surfcorr_gmm_inhomogeneity(recon_packet(mask_idx),n_cluster,profile_packet);
    end;
