...
if(flag_nufft)
    ...
    N1 = size(Y,1);    % Size of a two dimensional image
    N2 = size(Y,2);
...
for i=1:size(Y,3)
    ...
    if(flag_regrid)
        if(flag_regrid_kb)
            ...
            if(isempty(tra_regrid_kernel)|isempty(tra_regrid_kernel_inv))
                fprintf('regridding...\n');
                [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'flag_inv',0,'w',2,'b',4);
                recon=reshape(recon,[n_phase,n_freq]);
            else
                [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'kernel',tra_regrid_kernel,'kernel_inv',tra_regrid_kernel_inv,'flag_inv',0,'w',2,'b',4);
                recon=reshape(recon,[n_phase,n_freq]);
            end;
        end;
        Y(:,:,i)=recon;
    ...
    %regrid: roll-off for k-space trajectory
    if(flag_regrid)
        if(flag_regrid_kb)
            x_in=[tra_k_freq(:),tra_k_phase(:)];
            y_in=Temp(:);
