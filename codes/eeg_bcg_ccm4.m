function [eeg_bcg, qrs_i_raw]=eeg_bcg_ccm4(eeg,ecg,fs,varargin)
...
for i=1:length(varargin)/2
    ...
    switch lower(option)
        ...
        case 'n_ecg'
            n_ecg=option_value;
        case 'qrs_phase_limit'
            qrs_phase_limit=option_value;
        otherwise
...
%     end;
    ecg_ccm_idx(ii,:)=tmp+ecg_onset_idx(ii);  
end;
...
%    ccm_IDX(ecg_onset_idx(ii):ecg_offset_idx(ii),:)=repmat(ecg_onset_idx(IDX(ii,:)),[ecg_offset_idx(ii)-ecg_onset_idx(ii)+1,1])+repmat([0:ecg_offset_idx(ii)-ecg_onset_idx(ii)]',[1,nn]);
    ...
    %%%interpolation from the EEG instants of the same cardiac phase and the nearest EKG dynamics
    for nn_idx=1:nn
        ccm_IDX(ecg_onset_idx(ii):ecg_offset_idx(ii),nn_idx)=round(interp1([0:ecg_offset_idx(IDX(ii,nn_idx))-ecg_onset_idx(IDX(ii,nn_idx))]./(ecg_offset_idx(IDX(ii,nn_idx))-ecg_onset_idx(IDX(ii,nn_idx))),[ecg_onset_idx(IDX(ii,nn_idx)):ecg_offset_idx(IDX(ii,nn_idx))],[0:ecg_offset_idx(ii)-ecg_onset_idx(ii)]./(ecg_offset_idx(ii)-ecg_onset_idx(ii))));
    end;
...
%             norm(EEG_model-EEG_actual).^2/norm(EEG_actual).^2;
%             abs(EEG_actual);
%             if((norm(EEG_model-EEG_actual).^2/norm(EEG_actual).^2>0.8)&(abs(EEG_actual)>30))
%                 flag_debug=3;
%             else
%                 flag_debug=0;
%             end;
