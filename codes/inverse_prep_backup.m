...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%get various noise covariance files
%sensor noise covariance matrix
%C_ncov=inverse_read_ncov(file_noisecovariance,n_eeg,n_meg,nctype);
%get noise covariance of empty room from FIF file
%if(~isempty(file_noisecovariance_null_fif))
%	fprintf('calculating null condition noise covariance using [%s]!\n',file_noisecovariance_null_fif);
	C_null=inverse_get_C_fif(file_noisecovariance_null_fif,'null');
%end;
%get noise covariance of evoked response from FIF file
%if(~isempty(file_noisecovariance_evoke_fif))
%	fprintf('calculating evoked (pre-stimuli) noise covariance using [%s]!\n',file_noisecovariance_evoke_fif);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%if((~isempty(file_measure_mat_filter))&(~isempty(file_measure_mat_variable))&(~isempty(baseline_index)))
%    fprintf('\n\n%d. BASELINE NOISE COVARIANCE...\n',process_id);
%    process_id=process_id+1;
%    
%    dd=dir(file_measure_mat_filter);
%    fprintf('total [%d] data files...\n',length(dd));
%    fprintf('length of baseline interval=[%d]\n',length(baseline_index));
%    
%    
%    load(dd(1).name,file_measure_mat_variable);
%    eval(sprintf('tmp=%s(%s,:);',file_measure_mat_variable,mat2str(baseline_index)));
%    cc=zeros(size(tmp,1)*length(dd),size(tmp,2));
%
%    for ii=1:length(dd)
%       fn=dd(ii).name;
%        fprintf('loading [%s]...\n',fn);
%        load(fn,file_measure_mat_variable);
%        eval(sprintf('cc(%d:%d,:)=%s(%s,:);',1+(ii-1)*size(tmp,1),ii*size(tmp,1),file_measure_mat_variable,mat2str(baseline_index)));
%    end;
...
varargout{3}=C_ncov;
fprintf('output[%d]--> noise covariance from NCOV file [C_noise_norm]\n',output_count);
output_count=output_count+1;

varargout{4}=C_evoke;
fprintf('output[%d]--> noise covariance from evoked FIF file [C_evoke]\n',output_count);
output_count=output_count+1;

varargout{5}=C_null;
fprintf('output[%d]--> noise covariance from empty room FIF file [C_null]\n',output_count);
output_count=output_count+1;
