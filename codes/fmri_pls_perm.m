function [p_brainlv_perm,p_sv_perm]=fmri_pls_perm(datamat,contrast,PERMUTATION,datamat_struct,perm_flag,fname)
%fmri_pls_perm 		permutation test of the PLS analysis of functional data
%
%[brainlv_perm,sv_perm]=fmri_pls_perm(datamat,contrast,perm,datamat_struct,perm_flag,fname)
...
perm_datamat=zeros(size(datamat));
...
for i=1:PERMUTATION
	...
	case '111'
		...
		perm_datamat(j,:)=datamat(para2(j),:);
	end;
	
	[perm_brainlv,perm_sv,perm_designlv,perm_brain_score,perm_design_score]=fmri_pls_core(perm_datamat,contrast);
