...
elseif(~isempty(K_general))
    ...
    E_k=zeros(size(K_general,1),size(k_prep,1));
...
E=[];
E=zeros(size(E_k,1)*size(S,3),size(E_k,2));
for i=1:size(S,3)
    % S (sensitivity)
    ss=S(:,:,i);
    ss=ss(:);
    %E=cat(1,E,E_k*diag(ss));
    %E((i-1)*size(E_k,1)+1:i*size(E_k,1),:)=E_k*diag(ss);
    E((i-1)*size(E_k,1)+1:i*size(E_k,1),:)=E_k.*(ones(size(E_k,1),1)*(ss.'));
end;
