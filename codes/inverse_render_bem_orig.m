...
if(~isempty(file_fwd))
	...
	if(isempty(Y))
		inverse_bem_sensor=patch('Faces',FM',...
			'Vertices',V(:,1:3).*.95,...
			'EdgeColor','none',...
			'FaceColor',[1 1 1].*0.3,...
		...
		inverse_timecourse_x=t_idx;

		inverse_bem_sensor=patch('Faces',FM',...
			'Vertices',V(:,1:3).*.95,...
			'EdgeColor','none',...
			'FaceColor','interp',...
			'FaceVertexCData',Y(1+coil_offset:3:end,t_idx),...
			'SpecularStrength' ,0.1, 'AmbientStrength', 1.2,...
			'DiffuseStrength', 1);
	end;
	...
	for p=1:size(V,1)
		phi=atan(sqrt(V(p,1)^2+V(p,2)^2)/V(p,3));
		the=atan2(V(p,1),V(p,2));
    ...
    set(gcf,'WindowButtonDownFcn','inverse_render_bem_handle(''bd'')');
    set(gcf,'KeyPressFcn','inverse_render_bem_handle(''kb'')');
