function [inverse_out]=inverse_lcmv_core(varargin)
...
%     %Matrix_ARAt=(A.*repmat(R,[size(A,1),1]))*A';
    ...
    %RR=1./sum(W.*transpose(A),2);
    %RR=repmat(RR(:),[1,size(W,2)]);
    ...
    if(flag_noise_normalized_iop)
    	if(nperdip==3)
    		for idx=1:size(W,1)./nperdip
	 		WW=W_lcmv((idx-1)*nperdip+1:idx*nperdip,:);
    ...
    if(flag_whiten)
    	...
    	W_lcmv=W_lcmv*sqrt(diag(1./(ss_C)))*vv_C';
    end;
...
if(~isempty(W_lcmv))
    ...
    inverse_out{output_count}=W_lcmv;
    fprintf('output[%d]--> lcmv inverse operator\n',output_count);
end;
