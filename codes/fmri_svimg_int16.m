function [] = fmri_svimg_int16(data,imgfile,vox)
%fmri_svimg_int16	save 3d image data into 16-bit integer img file format (ANALYZE)
%
%fmri_svimg_int16(data,imgfile,vox)
%
...
maxx=ceil(max(max(max(data))));
minn=floor(min(min(min(data))));
bfile_data=data;
...
if TYPE == 4;   bitpix = 16; glmax = maxx;    glmin = minn;  	end
