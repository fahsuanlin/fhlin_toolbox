function [eeg_bcg, qrs_i_raw, bcg_all, ecg_all, bad_trials]=eeg_bcg_ssp(eeg,ecg,fs,varargin)
...
for ch_idx=1:length(non_ecg_channel)
    ...
    bad_trials{ch_idx}=zeros(1,length(qrs_i_raw));
    bad_trials{ch_idx}(bad_trial_idx)=1;
end;

bad_trials_idx_all=[];
for ch_idx=1:length(non_ecg_channel)
    if(ch_idx==1)
        bad_trials_idx_all=find(bad_trials{ch_idx});
    else
        bad_trials_idx_all=union(bad_trials_idx_all,find(bad_trials{ch_idx}));
    end;
end;
for ch_idx=1:length(non_ecg_channel)
...
tmp=[];
for ch_idx=1:length(non_ecg_channel)
    d=bcg_all{non_ecg_channel(ch_idx)};
    tmp(ch_idx,:)=d(:).';
...
return;
%remove BCG by SVD at each channel
for ch_idx=1:length(non_ecg_channel)
