...
%
%	pmri_grappa2_prep		prepare for GRAPPA2 reconstruction
%
%
%	[G]=pmri_grappa2_prep('obs',obs,'sample_vector',sample_vector,'acc_vector',acc_vector, 'flag_display', 1);
%
%	INPUT:
...
%		n_chan: # of channel
%	sample_array: 2D array with entries of 0 or 1: [n_PE1, n_PE2].
...
%	acc_vector: 2D array: [n_PE1, n_PE2].
%		n_PE1: # of phase encoding steps before acceleration
%		n_PE2: # of phase encoding steps before acceleration
...
acc_vector=reshape(acc_array,[size(acc_array,1)*size(acc_array,2),1]);
...
%for i=5:5
    ...
    %if(i==1) %A matrix is only required to prepared by once.
    ...
    if(flag_common_coil)
        ...
        for coil_idx=1:size(obs,3)
            ...
            if(rank(A)<size(A,2)|flag_reg_prep)
                %fprintf('#');
                snr=3;
                C=eye(size(A,1));
                lambda=trace(A*A')./trace(C)./snr;
                beta=A'*inv(A*A'+lambda.*C)*Y;
            else
                %if(i==1) inv_A=inv(A'*A)*A'; end;
                if(flag_irls_prep)
                    for idx=1:20
                        fprintf('#');
                        if(idx==1)
                            beta(:,idx)=inv(A'*A)*A'*Y;
                        else
