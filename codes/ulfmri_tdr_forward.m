...
for readout_idx=1:length(K_general)
    ...
    E_tmp=zeros(n_phase*n_freq,4,4);
    for idx=1:size(X_tmp,1)
        ...
        E_tmp(idx,:,:)=squeeze(R_pre(idx,:,:))'*squeeze(P_pre(idx,:,:))*squeeze(E_pre(idx,:,:))*squeeze(R_pre(idx,:,:));
    ...
    for t_idx=1:length(K_general(readout_idx).readout_time)
        ...
        for idx=1:size(X_tmp,1)
            ...
            E_tmp(idx,:,:)=squeeze(R(idx,:,:))'*squeeze(P(idx,:,:))*squeeze(E(idx,:,:))*squeeze(R(idx,:,:))*squeeze(E_tmp(idx,:,:));
        ...
        for ch_idx=1:size(S,4)
            ...
            ee=zeros(1,n_phase*n_freq*4);
            for idx=1:size(X_tmp,1)
               ee((idx-1)*4+1:idx*4)=dummy(:,idx).'*squeeze(E_tmp(idx,:,:));
            end;
            %Enc=cat(1,Enc,dummy(:).'*E_tmp);
            Enc=cat(1,Enc,ee);
        end;
