...
if(flag_mne_toolbox)
    if(isempty(mne_meg_channel_name))
        ...
        for ch=1:306
            tmp=fgetl(fp);
            tmp(find(isspace(tmp)))=[];
            mne_meg_channel_name{ch}=tmp;
        end;
        fclose(fp);
    end;
    ...
    if(flag_mne_toolbox)
        ...
        for idx=1:length(fwd.sol.row_names)
            if(~isempty(findstr(fwd.sol.row_names{idx},'EEG')))
                ...
                tmp=fwd.sol.row_names{idx};
                tmp(find(isspace(tmp)))=[];
                mne_channel{end+1}=tmp;
            end;
            for ch_idx=1:length(mne_eeg_channel_name)
                tmp=fwd.sol.row_names{idx};
                tmp(find(isspace(tmp)))=[];
                if(strcmp(tmp,mne_eeg_channel_name{ch_idx}))
    ...
    if(flag_mne_toolbox)
        ...
        for idx=1:length(fwd.sol.row_names)
            ...
            for ch_idx=1:length(mne_meg_channel_name)
                tmp=fwd.sol.row_names{idx};
                tmp(find(isspace(tmp)))=[];
                if(strcmp(tmp,mne_meg_channel_name{ch_idx}))
...
if(flag_ssp|~isempty(file_ssp_fiff))
    if(flag_mne_toolbox)
        ...
        [ fid, tree ] = fiff_open(file_ssp_fiff);
        ...
        for ii=1:length(mne_channel)
            for jj=1:length(raw.info.ch_names)
                tmp=raw.info.ch_names{jj};
                tmp(find(isspace(tmp)))=[];
                if(strmatch(tmp,mne_channel{ii},'exact'))
                    pick=cat(1,pick,jj);
                    break;
                end;
            end;
...
varargout{output_count}=R;
fprintf('output[%d]--> prior [R]\n',output_count);
output_count=output_count+1;

if(~isempty(C_ncov))
    varargout{output_count}=C_ncov;
    fprintf('output[%d]--> noise covariance from NCOV file [C_noise_norm]\n',output_count);
    output_count=output_count+1;
end;

if(~isempty(C_evoke))
    varargout{output_count}=C_evoke;
    fprintf('output[%d]--> noise covariance from evoked FIF file [C_evoke]\n',output_count);
    output_count=output_count+1;
end;

if(~isempty(C_null))
    varargout{output_count}=C_null;
    fprintf('output[%d]--> noise covariance from empty room FIF file [C_null]\n',output_count);
    output_count=output_count+1;
end;

%if(~isempty(BAD))
varargout{output_count}=BAD;
fprintf('output[%d]--> bad channel data structure\n',output_count);
output_count=output_count+1;
%end;


%if(~isempty(SSP))
varargout{output_count}=SSP;
fprintf('output[%d]--> SSP matrix\n',output_count);
output_count=output_count+1;
%end;

%if(~isempty(mne_channel))
varargout{output_count}=mne_channel;
fprintf('output[%d]--> mne_channel\n',output_count);
output_count=output_count+1;
%end;
