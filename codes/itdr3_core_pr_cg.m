function [recon,b,delta]=itdr3_core_pr_cg(varargin);
%
%	itdr3_core_pr_cg		perform SENSE reconstruction using conjugated gradient method
%
%
%	[recon,b,delta]=itdr3_core_pr_cg('Y',Y,'S',S,'C',C,'K',K,'flag_display',1);
%
%	INPUT:
...
for y_idx=1:n_phase
    ...
    for i=1:size(Y,3)
        X=repmat(transpose(Y(y_idx,:,i)),[1 n_phase n_freq]);
        X=permute(X,[2 3 1]);
...
while(~convergence)
    ...
    if(sum(dd(:))<epsilon)
        ...
        for y_idx=1:n_phase
            ...
            for i=1:size(Y,3)
                X=repmat(transpose(buffer(y_idx,:,i)),[1 n_phase n_freq]);
                X=permute(X,[2 3 1]);
                recon(:,:,i)=recon(:,:,i)+sum(X.*K_freq.*K_phase,3);
            end;
        end;
