...
for idx=1:length(non_sample_idx)
    ...
    %check if the ACC data entries are outside the k-space boundary
    ...
    %get corresponding ACC data entries
    ...
    if(isempty(acc_acs_idx))
        ...
        if(flag_display)
            fprintf('grappa cs recon. data entry [[%04d]] <<%04d|%04d>> (%2.2f%%)...',non_sample_idx(idx),idx,length(non_sample_idx),idx./length(non_sample_idx).*100);
        end;

        A_acs=[];

	max_acs_candidate_yy=max(acs_candidate_yy(acc_acs_idx));
	min_acs_candidate_yy=min(acs_candidate_yy(acc_acs_idx));
	max_acs_candidate_xx=max(acs_candidate_xx(acc_acs_idx));
	min_acs_candidate_xx=min(acs_candidate_xx(acc_acs_idx));
	max_acs_candidate_yy=max([max_acs_candidate_yy acs_center_y]);
	min_acs_candidate_yy=min([min_acs_candidate_yy acs_center_y]);
	max_acs_candidate_xx=max([max_acs_candidate_xx acs_center_x]);
	min_acs_candidate_xx=min([min_acs_candidate_xx acs_center_x]);

	min_acs_candidate_xx=min_acs_candidate_xx-acs_center_x;
	min_acs_candidate_yy=min_acs_candidate_yy-acs_center_y;
	max_acs_candidate_xx=max_acs_candidate_xx-acs_center_x;
	max_acs_candidate_yy=max_acs_candidate_yy-acs_center_y;

	for xx_shift=-prior_half_acs(2)-min_acs_candidate_xx: prior_half_acs(2)-max_acs_candidate_xx
		for yy_shift=-prior_half_acs(1)-min_acs_candidate_yy: prior_half_acs(1)-max_acs_candidate_yy

		        for k=1:size(freq_idx,2)
        		    T=[];
		            for tmp_idx=1:length(acc_acs_idx)
		                A_acs_tmp=obs(acs_candidate_yy(acc_acs_idx(tmp_idx))+yy_shift,acs_candidate_xx(acc_acs_idx(tmp_idx))+xx_shift,freq_idx(:,k),:);
	...
	for xx_shift=-prior_half_acs(2)-min_acs_candidate_xx: prior_half_acs(2)-max_acs_candidate_xx
		for yy_shift=-prior_half_acs(1)-min_acs_candidate_yy: prior_half_acs(1)-max_acs_candidate_yy
		            ...
		            Y_acs_tmp=obs(acs_center_y+yy_shift,acs_center_x+xx_shift,freq_select_idx,target_coil);
		            Y_acs_tmp=permute(Y_acs_tmp,[3 1 2 4]);
			    Y_acs=cat(1,Y_acs,Y_acs_tmp);
		end;
	end;
