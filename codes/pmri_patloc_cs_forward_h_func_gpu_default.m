...
elseif(pmri_cs_obj.patloc_obj.flag_vec2)
    ...
    if(pmri_cs_obj.flag_gpu)
	recon_buffer_gpu=gdouble(gzeros(pmri_cs_obj.patloc_obj.n_phase*pmri_cs_obj.patloc_obj.n_freq,pmri_cs_obj.n_chan));
    end;
    for g_idx=1:length(pmri_cs_obj.patloc_obj.G)
	...
	elseif(pmri_cs_obj.flag_gpu)
	    ...
	    %	[rr,cc]=ind2sub(k_size_gpu,gsingle(kspace(k_idx)));
	    %	cc1=cc-floor(n_freq_gpu./2)-1;
	    %	rr1=rr-floor(n_phase_gpu./2)-1;
	    %	phi=repmat(K_freq_gpu.*cc1+K_phase_gpu.*rr1,[1 pmri_cs_obj.n_chan]);
	    %	recon_buffer_gpu=recon_buffer_gpu+exp(sqrt(-1).*(-phi))*diag(X_gpu(kspace(k_idx),:));
	    %end;
            g1=gdouble(ones(1,pmri_cs_obj.n_chan));
	    gfor k_idx=(1:length(kspace))
		dd=kspace_gpu(k_idx);
		[rr,cc]=ind2sub(k_size,dd);
		cc1=cc-floor(n_freq_gpu./2)-1;
		rr1=rr-floor(n_phase_gpu./2)-1;
		phi=(K_freq_gpu.*cc1+K_phase_gpu.*rr1)*g1;
		recon_buffer_gpu=recon_buffer_gpu+exp(sqrt(-1).*(-phi))*diag(X_gpu(double(dd),:));
	    gend; 

	    recon_buffer=gforce(double(recon_buffer_gpu));
        else
            kspace=find(pmri_cs_obj.patloc_obj.K{g_idx}>eps);
