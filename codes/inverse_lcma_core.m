function [inverse_out]=inverse_lcma_core(varargin)
...
while(((focus_convergence>focus_limit_convergence)&(focus_iteration<=focus_limit_iteration)&flag_focus)|(~flag_focus))
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%
    %	creating data covariance matrix
    %%%%%%%%%%%%%%%%%%%%%%%%%%%
    fprintf('\n\n%d. DATA COVARIANCE MATRIX...\n',process_id);
    process_id=process_id+1;
    
    if(isempty(D)&~isempty(Y))
        D=Y*Y'./size(Y,2);
    end;
    ...
    ss=1./s(1:nn);
    ss=diag(ss);
    Matrix_SN_inv=u(:,1:nn)*ss(1:nn,1:nn)*u(:,1:nn)';
    
    ssq=sqrt(s(1:nn));
    ssq=diag(ssq);
    D_half_inv=u(:,1:nn)*ssq(1:nn,1:nn);
    %[u,s,v]=svd(D);
    %D_half_inv=u*sqrt(s);   
    ...
    %W_num=Matrix_SN_inv*A;
    %W_den=repmat(sum(conj(A).*W_num,1),[size(A,1),1]);
    
    
    %W=transpose(W_num./W_den);
    
    
    % Builds and solves a simple linear program
    
    nn=size(A,1);
    W_lcma=zeros(size(A,2),size(A,1));
    DD=D_half_inv(1:size(D_half_inv,2)-1,:);
    %DD=diag(diag(D_half_inv(1:298,1:298)));
    
    for n_idx=10342:10344
    %for n_idx=1:size(A,2)
        aa=A(:,n_idx);
        %aa0=A(1:size(D_half_inv,2)-103,n_idx);
        %aa0=A(1:298,n_idx);
        
        cvx_solver sdpt3
        cvx_begin
            %variable x(size(A,1),1)
            %variable x(size(aa0))
            variable x(size(aa))
            
            minimize( norm(x'*D_half_inv ,1) ) %lcma
            %minimize( norm(x'*DD ,1) ) %lcma
            %minimize( x'*Matrix_SN*x  ) %lcmv
            subject to
                %abs(aa0'*x-1)<100;
                %abs(aa0'*x-1)<0.01;
                %norm(aa0'*x-1)< 0.01;
        ...
        %aa'*x
        %keyboard;
        %W_lcma(n_idx,1:298)=x(:)';
        W_lcma(n_idx,1:length(x))=x(:)';
        %keyboard;
    ...
    if(flag_noise_normalized_iop)
        if(nperdip==3)
            for idx=1:size(W_lcma,1)./nperdip
                WW=W_lcma((idx-1)*nperdip+1:idx*nperdip,:);
    ...
    if(flag_whiten)
        ...
        W_lcma=W_lcma*sqrt(diag(1./(ss_C)))*vv_C';
    end;
...
if(~isempty(W_lcma))
    ...
    inverse_out{output_count}=W_lcma;
    fprintf('output[%d]--> lcma inverse operator\n',output_count);
end;
