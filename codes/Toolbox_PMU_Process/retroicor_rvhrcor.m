function [clean,card,resp] = retroicor_rvhrcor(data_epi,data_ecg,data_resp,dt,mc,mr)
% global phiC phiR data_epi ac bc ar br clean
...
%
% In:
%   data_epi  - VxN matrix with EPI data
...
%
% Out:
%   clean - VxN matrix of cleaned EPI data (y-y_delta)
%   card  - VxN matrix of cardiac contribution
%   resp  - VxN matrix of respiratory contribution
%
% Description:
...
%% Do the retroicor estimation per voxel
  ...
  % Loop through all pixels
      for i=1:vdim
        ...
        % Mean
        ybar = mean(data_epi(i,:));      

        % Cardiac
        for m=1:mc
          ac(m,i) = sum(squeeze(data_epi(i,:)-ybar).*cos(m*phiC))/ ...
            sum(cos(m*phiC).^2);
          bc(m,i) = sum(squeeze(data_epi(i,:)-ybar).*sin(m*phiC))/ ...
            sum(sin(m*phiC).^2);
        end
      
        % Respiratory
        for m=1:mr
          ar(m,i) = sum(squeeze(data_epi(i,:)-ybar).*cos(m*phiR))/ ...
            sum(cos(m*phiR).^2);
          br(m,i) = sum(squeeze(data_epi(i,:)-ybar).*sin(m*phiR))/ ...
            sum(sin(m*phiR).^2);
        end
        ...
        % Show time remaining
          l = (i-1)*vdim;
          if rem(l,100)==0 
            secondsleft = min(toc(loopstart),Inf)/l*(vdim-l);
...
%% Output signals
  ...
  for i=1:vdim    
        % Cardiac
        for m=1:mc
          card(i,:) = card(i,:) + ac(m,i)*cos(m*phiC) + ...
                                             bc(m,i)*sin(m*phiC);%%change -2 to -1
        end
      
        % Respiratory
        for m=1:mr
          resp(i,:) = resp(i,:) + ar(m,i)*cos(m*phiR) + ...
                                             br(m,i)*sin(m*phiR); 
        end
  end
