function [EF_count, timeVec, reject_counter, EF_count_all]=inverse_scan_fif(inputname,Trigger,Chans,TimeInt,TimeExtr,varargin) 
...
TRACE = zeros(size(B,1),colTRACE); 
            ...
            if((flag_meg)&(~flag_eeg)) 
                reject_counter= zeros(1,length(rowMEG));
            end;
            if((~flag_meg)&(flag_eeg)) 
                reject_counter= zeros(1,length(rowEEG));
            end;
            if((flag_meg)&(flag_eeg)) 
                reject_counter= zeros(1,length(rowEEG)+length(rowMEG));
            end;
%------------------------------------------------------------------------
...
while strcmp(status,'ok') & tCurrent < tStop 
    ...
    for k=1:length(TrigList)
        if colPre+TrigList(k)+colPost <= size(TRACE,2)  
            ...
            if(flag_auto_reject)
                %                [tmpVal,dFmaxCh] = max(max(diff(abs(Ttmp(good_channel,:)'))));
                [tmpVal,dFmaxCh] = max(diff(abs(Ttmp(good_channel,:)')));
                DFDTmax = 1e13*tmpVal./(1/Fs);
            else
                DFDTmax=0.0;
            end;
            ...
            if(flag_auto_reject)
                %                [tmpVal,FmaxCh] = max(max(abs(Ttmp(good_channel,:)')));
                [tmpVal,FmaxCh] = max(abs(Ttmp(good_channel,:)'));
                Fmax = 2*1e13.*tmpVal;
            else
                Fmax=0.0;
            end;
            ...
            DFDTmax_idx=find(DFDTmax > InitParam.DFDTreject); 
            if (~isempty(DFDTmax_idx))
                DFDTrej = DFDTrej + 1;
                reject_str='';
                for ii=1:length(DFDTmax_idx)
                    reject_str=strcat(reject_str,sprintf('[%s] ',char(ChNames(DFDTmax_idx(ii)))));
                end;
                fprintf('Reject:%s(dF) -\n',reject_str);
                traceOK = 0;
                reject_counter(DFDTmax_idx)=reject_counter(DFDTmax_idx)+1;
            end
            
            %            if Fmax > InitParam.Freject 
            Fmax_idx=find(Fmax > InitParam.Freject);
            if (~isempty(Fmax_idx))
                Frej = Frej + 1;
                reject_str='';
                for ii=1:length(Fmax_idx)
                    reject_str=strcat(reject_str,sprintf('[%s] ',char(ChNames(Fmax_idx(ii)))));
                end;
                fprintf('Reject:%s(F)-\n',reject_str);
                traceOK = 0;
                reject_counter(Fmax_idx)=reject_counter(Fmax_idx)+1;
            end
