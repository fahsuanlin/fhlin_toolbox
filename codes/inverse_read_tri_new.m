...
if(flag_nmr);
	...
	[vertex(:,1),vertex(:,2),vertex(:,3),d1,d2,d3]=textread(tri_file,'%f%f%f%f%f%f',n_vertex,'headerlines',1);


	n_face=textread(tri_file,'%d',1,'headerlines',n_vertex+1);
	fprintf('[%d] triangulations...\n',n_face);
	face=zeros(n_face,3);
	[face(:,1),face(:,2),face(:,3)]=textread(tri_file,'%d%d%d',n_face,'headerlines',2+n_vertex);
