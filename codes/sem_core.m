...
if(flag_mi)
    dof=sem.n_obs; %degree of freedom
    [mi,pi]=sem_obj_mi(path_val_free_opt, sem.path_val(find(sem.path_fix)), find(~sem.path_fix), find(sem.path_fix), sem.A, sem.S, sem.F, sem.covv, dof,'obj_type', obj_type);
end;

%V=inv(SS);
%f=0.5.*trace((CC-SS)*V).^2
