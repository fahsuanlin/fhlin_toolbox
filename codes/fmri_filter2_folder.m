function []=fmri_filter2_folder(folder,mask)
%fmri_filter2_folder	average the raw data in a folder by 2D mask
%
%[]=fmri_filter2_folder(folder,mask)
%
...
for j=1:b
	...
	disp(s);
	filter_data=fmri_filter2(buffer,mask);
		
		
	fn=sprintf('fil_%s',char(f(j)));
