function [R,AA,RR]=pmri_grappa_core(varargin)
%
...
for i=1:length(acc_token)
    ...
    if(flag_2d)
        acs2d_block_shift=G{i}.acs2d_block_shift;
        acs2d_block_shift_scale_x=G{i}.acs2d_block_shift_scale_x;
        acs2d_block_shift_scale_y=G{i}.acs2d_block_shift_scale_y;
    end;
    ...
    if(isempty(acs_vector))
        acs_idx=intersect(find(acc_vector==acc_token(i)),find(1-sample_vector));
    else
    ...
    for j=1:length(acs_idx)
        ...
        %if(j==1) AA{i}=zeros(length(acs_idx)*size(A,1),size(A,2)); end;
        if(j==1) AA{i}=[]; end;
        AA{i}=cat(1,AA{i},A);
