function [ROC_X,ROC_Y,auc]=etc_roc(x,source,varargin)
...
for th_idx=1:length(threshold)
    if(flag_display) fprintf('[%d|%d] threshold = %2.2e...',th_idx,length(threshold), threshold(th_idx)); end;
    ...
    if(flag_display) fprintf('TPR=%2.2f%%\tFPR=%2.2f%%\r',tpr(th_idx)*100,fpr(th_idx)*100); end;
end;
...
if(flag_display) plot(ROC_X, ROC_Y,'b'); hold on; plot(fpr,tpr,'r.'); end;
