...
file_badchannel='chan.bad';					%you can set it to '' (empty string)
%file_badchannel='';


file_noisecovariance='noise.ncov';				%you can set it to '' (empty string)
...
file_measure_mat='iiro_devshighdev_meg.mat';

file_output='iiro';				%output file stem; Thiis is for IOP and STC file.

file_brain_patch={
'e:/user/fhlin/workspace/subjects/matlab_render/iiro_brain_inflated_lh_full.mat';
'e:/user/fhlin/workspace/subjects/matlab_render/iiro_brain_inflated_rh_full.mat';
};
...
dipole_filter_width=[1 10];				%transition frequency (Hz) of bandpass filter for the estimated dipole 
...
flag_render_stc=1;					%render STC into AVI file		
...
if(flag_write_stc)
	...
	if(~isempty(who('X')))
		...
		for i=1:length(dec)
			...
			x{i}=X(offset+1:offset+length(dec{i}),:);
			...
			x{i}=fmri_scale(x{i},10,0);	%scale the dipole estimates between 10 and 1
			offset=offset+length(dec{i});

			inverse_write_stc(x{i},dec{i}-1,init_latency,sample_time,file_stc); 
		end;
	else
...
if(flag_render_stc)
	...
	for i=1:1%only left hemisphere
		stc=x{i};
        stc=stc(:,[101:150]);
        for j=1:10
            ss(:,j)=mean(stc(:,(j-1)*5+1:j*5),2);
        end;

		fn=sprintf('%s_%02d.avi',file_output_stem,i);
        inverse_render_stc(file_brain_patch{i},abs(ss),dec{i}-1,[],dec{i}-1,'avi_output_file',fn,'view_angle',[-270+180*i,0],'grid','off');
	end;
else
