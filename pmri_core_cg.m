function [recon,b,delta,g2_gfactor]=pmri_core_cg(varargin);
%
...
while(~convergence)
	...
	if(dd<epsilon)
		...
		if(iteration_idx==2&flag_cg_gfactor)
			ss_gfactor=Y(:,:,1);
		else
			ss_gfactor=[];
		end;
		...
		for i=1:n_chan
			...
			if(~isempty(ss_gfactor))
				temp_gfactor(:,:,i)=fftshift(ifft2(fftshift(temp_gfactor(:,:,i))));
			end;
            
			buffer1=zeros(size(temp(:,:,i)));
			buffer0=temp(:,:,i);
			buffer1(k_idx)=buffer0(k_idx);
			temp(:,:,i)=buffer1;

			if(~isempty(ss_gfactor))
				buffer1=zeros(size(temp_gfactor(:,:,i)));
				buffer0=temp_gfactor(:,:,i);
				buffer1(k_idx)=buffer0(k_idx);
				temp_gfactor(:,:,i)=buffer1;
			end;
			...
			if(~isempty(ss_gfactor))
				temp_gfactor(:,:,i)=fftshift(fft2(fftshift(temp_gfactor(:,:,i))));
			end;
			...
			if(~isempty(ss_gfactor))
          			temp_gfactor(:,:,i)=temp_gfactor(:,:,i).*conj(S(:,:,i));
			end;
		end;
