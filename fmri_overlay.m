    ...
    for i=1:length(varargin)/2
        ...
        switch lower(option)
            ...
            case 'tr'
                TR=option_value;
            case 'beta'
                fmri_beta=option_value;
            case 'hdr'
                fmri_hdr=option_value;
            case 'timevec'
                fmri_timeVec=option_value;
            case 'contrast'
...
if(length(size(img))==3)
    [y,x,z]=size(img);
    [dummy,img]=fmri_mont(reshape(img,[y,x,z]),montage_config,'null');
...
if(length(size(overlay))==3)
    [y,x,z]=size(overlay);
    [dummy,overlay]=fmri_mont(reshape(overlay,[y,x,1,z]),montage_config,'null');
end;
...
switch mode
    case 1,
        ...
        if(isempty(ccmap))
            ...
            overlay_map=imresize(ccmap,[overlay_depth,size(colormap,2)]);
        end;
        fmri_colormap=overlay_map;
...
set(fmri_fig_overlay,'name','fmri_fig_overlay');
