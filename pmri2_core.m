function [x_unreg, g_unreg, x_reg, g_reg]=pmri2_core(varargin);
%
%	pmri2_core		perform SENSE/Inverse reconstruction for 2D parallel
%	MRI
%
%
%	[recon_unreg, g_unreg, recon_reg, g_reg]=pmri2_core('K',K,'Y',Y,'S',S,'C',C);
%
%	INPUT:
%	K: 2D fourier aliasing matrix of [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%	Y: input data of [n_PE1, n_PE2, n_chan]. 
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%		n_chan: # of channel
%	S: coil sensitivity maps of [n_PE1, n_PE2, n_chan].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%		n_chan: # of channel
%	Omega:  phase maps of [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%	C: noise covariance matrix of [n_chan, n_chan].
%		n_chan: # of channel
%	P: prior of [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%	R: source covariance matrix of [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
...
%
%	OUTPUT:
%	recon_unreg: 2D un-regularized SENSE reconstruction [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%	g_unreg: 2D un-regularized g-factor map [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%	recon_reg: 2D regularized SENSE reconstruction [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%	g_reg: 2D regularized g-factor map [n_PE1, n_PE2].
%		n_PE1: # phase encoding steps in direction 1
%		n_PE2: # phase encoding steps in direction 2
%
%---------------------------------------------------------------------------------------
...
x_unreg=zeros(size(Y,1),size(Y,2));
x_reg=zeros(size(Y,1),size(Y,2));

g_unreg=zeros(size(Y,1),size(Y,2));
g_reg=zeros(size(Y,1),size(Y,2));
...
cont=1;
recon_source_index=zeros(size(Y,1),size(Y,2));
recon_obs_index=zeros(size(Y,1),size(Y,2));
recon_counter=0;
current_cc=1;
while(cont)
    ...
    if(isempty(obs_idx))
        ...
        psf=fftshift(fft2(fftshift(fftshift(ifft2(fftshift(tmp))).*K)));
        
        ttmp=abs(psf);
        source_idx=find(abs(ttmp(:))>eps);
        [rr,cc]=ind2sub([size(Y,1),size(Y,2)],source_idx);
        if(cc(1)>current_cc)
            if(flag_display)
                fprintf('.')
            end;
...
if(flag_display)
    fprintf('\n');
    tt=toc;
    fprintf('SENSE2 recon. time=%2.2f\n',tt);
end;
