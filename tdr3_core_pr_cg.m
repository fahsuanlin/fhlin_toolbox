function [recon,b,delta]=tdr3_core_cg(varargin);
%
%	tdr3_core_cg		perform SENSE reconstruction using conjugated gradient method
%
%
%	[recon,b,delta]=tdr3_core_cg('Y',Y,'S',S,'C',C,'K',K,'flag_display',1);
%
%	INPUT:
...
%preparation for PR trajectory regridding
if(~isempty(tra))
    tra_k_freq=tra{1};
    tra_k_phase=tra{2};
end;
...
tra_regrid_kernel=[];
tra_regrid_kernel_inv=[];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%preparation for nufft
if(flag_nufft)
    
    flag_regrid=0;
    
    N1 = n_phase;    % Size of a two dimensional image
    N2 = n_freq;


    % Parameters for the nufft
    method = 'minmax:kb';       % This should the fastest way to do nufft
    % and accurate enough as this example should reveal
    Nd = [N1 N2];               % Size of the image
    Kd = 2*Nd;                  % Some parameter for which the optimal value
    % with minmax method is 2*Nd.
    Jd = [5 5];                 % Number of neighbors used in interpolating the k-space
    % A higher number seems to be only lengthening
    % the computation time and not giving any
    % smaller error.
    n_shift = [N1/2 N2/2];            % A shift parameter that can be used to do fftshift
    % in which case it would be n_shift = [N1/2 N2/2]
    fx=grid_freq./(max(abs(grid_freq(:)))).*pi;
    idx=find(fx<0);
    fx(idx)=fx(idx)+2*pi;
    
    fy=grid_phase./(max(abs(grid_phase(:)))).*pi;
    idx=find(fy<0);
    fy(idx)=fy(idx)+2*pi;
    
    st = nufft_init([fx(:) fy(:)], Nd, Jd, Kd, n_shift, method);
...
for i=1:size(Y,3)
    ...
    %regrid for k-space trajectory
    if(flag_regrid)
        if(flag_regrid_kb)
            x_in=[tra_k_freq(:),tra_k_phase(:)];
            y_in=Temp(:);
            x_out=[xtra_k_freq(:),xtra_k_phase(:)];

            if(isempty(tra_regrid_kernel)|isempty(tra_regrid_kernel_inv))
                fprintf('regridding...\n');
                [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'flag_inv',0,'flag_pr',1);
                recon=reshape(recon,[n_phase,n_freq]);
            else
                [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'kernel',tra_regrid_kernel,'kernel_inv',tra_regrid_kernel_inv,'flag_inv',0,'flag_pr',1);
                recon=reshape(recon,[n_phase,n_freq]);
            end;
            Temp=recon;
        end;
    ...
    if(~flag_nufft)
        Temp=fftshift(ifft2(fftshift(Temp)));
    else
        img=Temp;
        Temp=nufft_adj(img(:),st)/(N1*N2);
    ...
    %regrid for k-space trajectory: roll-off
    if(flag_regrid)
        if(flag_regrid_kb)
            x_in=[tra_k_freq(:),tra_k_phase(:)];
            y_in=Temp(:);
            ...
            if(isempty(tra_regrid_kernel)|isempty(tra_regrid_kernel_inv))
                fprintf('regridding...\n');
                [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'flag_inv',1);
                recon=reshape(recon,[n_phase,n_freq]);
            else
                [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'kernel',tra_regrid_kernel,'kernel_inv',tra_regrid_kernel_inv,'flag_inv',1);
                recon=reshape(recon,[n_phase,n_freq]);
            end;
            Temp=recon;
       end;
...
while(~convergence)
    ...
    if(sum(dd(:))<epsilon)
        ...
        for y_idx=1:n_phase
            fprintf('#');
            D_freq=repmat(([1:n_freq]-floor(n_freq./2)-1)'.*cos(pi./n_phase.*(y_idx-1)),[1 n_phase n_freq]);
            D_freq=permute(D_freq,[2 3 1]);
            K_freq=exp(sqrt(-1).*(-1).*gamma.*grad_max_freq.*delta_time_freq.*D_freq.*FOV_freq./n_freq.*G_freq);

            D_phase=repmat(([1:n_phase]-floor(n_phase./2)-1)'.*sin(pi./n_phase.*(y_idx-1)),[1 n_phase n_freq]);
            ...
            for i=1:size(Y,3)
                ...
                Temp(y_idx,:,i)=squeeze(sum(sum(X.*K_freq.*K_phase,1),2));
            end;
        end;
        ...
        for i=1:size(Y,3)
            ...
            if(flag_regrid)
                if(flag_regrid_kb)
                    ...
                    if(isempty(tra_regrid_kernel)|isempty(tra_regrid_kernel_inv))
                        fprintf('regridding...\n');
                        [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'flag_inv',0);
                        recon=reshape(recon,[n_phase,n_freq]);
                    else
                        [recon, tra_regrid_kernel,tra_regrid_kernel_inv]=etc_regridn(x_in,y_in,x_out,'kernel',tra_regrid_kernel,'kernel_inv',tra_regrid_kernel_inv,'flag_inv',0);
                        recon=reshape(recon,[n_phase,n_freq]);
                    end;
                    Temp=recon;
                end;
