function [kvol, navs] = read_diff_meas(sliceNum, diffGradNum, acqNum, ...
				       varargin)
% Read meas.out for a standard diffusion sequence on Numaris 4. 
%
% [kvol, navs] = read_diff_meas(sliceNum, diffGradNum, acqNum, 
%                               [PHASECOR], [DISPLAY]);
...
for ff = 1:length(files)
  if strcmp(files(ff).name, 'meas_1.out')
    MEASOUT_FOUND = 1;
  end
end
if MEASOUT_FOUND == 0
...
% For now, hard code, later, read from mrprot.asc
...
adcNumber = ( (sliceNum-1)*(nNavs + nRows) + ...
	      (diffGradNum-1)*nSlices*(nNavs + nRows) + ...
	      (acqNum-1)*nDiffGrads*nSlices*(nNavs + nRows) );
...
USECENTERNAV = 1;
if USECENTERNAV
  phaseCorrection = angle( (0.5*(navs(1,65) + navs(3,65))) ...
			   ./navs(2,65) );
else
