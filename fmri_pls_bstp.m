function [z_pls_bstp,se_pls_bstp,z2_pls_bstp,se2_pls_bstp]=fmri_pls_bstp(datamat,contrast,BOOTSTRAP,datamat_struct,bstp_flag,fname)
%fmri_pls_bstp		bootstrap for the PLS analysis of functional data
%
%[z_bstp,se_bstp]=fmri_pls_bstp(datamat,contrast,parafile,bstp,datamat_struct,bstp_flag,fname)
...
bstp_datamat=zeros(size(datamat));
...
for i=1:BOOTSTRAP
	...
	case '111'
		...
		bstp_datamat(j,:)=datamat(para2(j),:);
	end;

	[bstp_brainlv,bstp_sv,bstp_designlv,bstp_brain_score,bstp_design_score]=fmri_pls_core(bstp_datamat,contrast);
