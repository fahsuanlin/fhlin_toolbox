...
elseif(ice_obj.flag_ini3d)
    ...
    for ch=1:size(ice_m_data,2)
        ...
        fmri_svbfile(real(squeeze(ice_m_data(:,ch,:))),fn);

        fn=sprintf('%s_chan%03d_im.bfloat',ice_obj.output_stem,ch);
        fmri_svbfile(imag(squeeze(ice_m_data(:,ch,:))),fn);
    end;
