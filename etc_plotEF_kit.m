function h=etc_plotEF_kit(data,varargin)
%
% etc_plotEF_kit    plot the evoked fields from KIT MEG
%
% h=etc_plotEF_kit(data,[option1, option_value1,...]);
%
...
if(isempty(hdr)) %default channel positions for KIT 157 gradiometer system
    ...
    hdr.grad.coilpos=[12.1393    2.4261    3.9180
...
meg_x=hdr.grad.coilpos(:,1);
meg_y=hdr.grad.coilpos(:,2);
meg_z=hdr.grad.coilpos(:,3);
...
meg_x_2d_orig=meg_x_2d;
meg_y_2d_orig=meg_y_2d;
meg_x_2d=-meg_y_2d_orig;
meg_y_2d=meg_x_2d_orig;
...
for h_idx=1:length(meg_x_2d)
    ...
    if(~isempty(data_aux))
        if(~iscell(data_aux))
            for ii=1:size(data_aux,3)
                h=plot(h_ax(h_idx),timeVec,data_aux(h_idx,:,ii)); set(h,'HitTest','off');
            end;
        else
            for ii=1:length(data_aux)
                h=plot(h_ax(h_idx),timeVec,data_aux{ii}(h_idx,:)); set(h,'HitTest','off');
            end;
        end;
    end;
